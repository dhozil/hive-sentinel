// Dashboard API server — menjembatani genlayer-js (Node) ke UI statis.
...
// Live deployed contracts on StudioNet — REAL on-chain targets, not mock
...
async function simulateAttack(body) {
  ...
  // Ephemeral gasless account per attack — the attacker address shown
  ...
  // Consensus on StudioNet can be slow — give validators plenty of time.
  let receipt = null;
  let waitError = null;
  try {
  ...
  // as PENDING instead of a hard FAILED — the verdict may still land.
...
server.listen(PORT, () => {
  console.log(`⚡ HIVE SENTINEL dashboard: http://localhost:${PORT}`);
});
